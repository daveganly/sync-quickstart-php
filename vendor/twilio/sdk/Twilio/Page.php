...
abstract class Page implements \Iterator {
    ...
    protected function processResponse(Response $response) {
        if ($response->getStatusCode() != 200) {
            throw new HttpException('Observed failure when paging: ' . json_encode($response->getContent()));
        } else if(!$this->isPagingEol($response->getContent())) {
            throw new DeserializeException('Unable to process page!');
        }
